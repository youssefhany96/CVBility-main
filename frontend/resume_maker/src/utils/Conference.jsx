...
export const ConferenceComponent = ({ id }) => {
  const {
    handleNewField,
    CurrentEntity,
    setCurrentEntity,
  ...
  const ConfdescrpRef = useRef();
  const ConfDateRef = useRef();
  const addConf = useRef();
  const handleAdd = () => {
    handleNewField("conference");
  };

  const handleOnEnter = (e) => {
    if (document.getElementById(`${id}ConfcourseId`).childNodes.length < 10) {
      if (e.key === "Enter") {
        e.preventDefault();
        ...
        if (e.target.innerHTML.replace(/&nbsp;/g, "").trim() != "") {
          ...
          let SingleCourse = document.getElementById(`${id}confCourse`);
  ...
  useEffect(() => {
    ...
    setConferenceData([...ConferenceData, newData]);
    if (+id > 1) {
      InstRef.current.focus();
    }
  }, [id]);
  useEffect(() => {
    if (+id > 0 && SelectedId == id && CurrentEntity == "conf") {
      const underliend = document
        .getElementsByClassName(`${id}conf`)[0]
        .querySelectorAll(".x");
      const underliendDash = document
        .getElementsByClassName(`${id}conf`)[0]
        .querySelectorAll(".y");
      document.getElementById(`${id}confCourse`).classList.remove("hidden");
      document.getElementById(`${id}confDate`).classList.remove("hidden");
      addConf.current.classList.remove("hidden");
      ...
      document.getElementById(`${id}confCourse`).classList.add("hidden");
      document.getElementById(`${id}confDate`).classList.add("hidden");
      addConf.current.classList.add("hidden");
    ...
    const handler = (e) => {
      const menu = document.getElementById("conferencemenu");
      ...
      if (document.getElementsByClassName(`${id}conf`)[0].contains(e.target)) {
        document.getElementById(`${id}confCourse`).classList.remove("hidden");
        document.getElementById(`${id}confDate`).classList.remove("hidden");
        addConf.current.classList.remove("hidden");
        for (let i of underliend) {
      ...
      if (
        ConfdescrpRef.current.innerHTML === "" &&
        !ConfdescrpRef.current.contains(e.target)
      ) {
        if (
          menu &&
          !menu.contains(e.target) &&
      ...
      const date = Array.from(ConfDateRef.current.children);

      if (
        (date[0].value === "" || date[1].value === "") &&
        !ConfDateRef.current.contains(e.target)
      ) {
        if (
          menu &&
          !menu.contains(e.target) &&
  ...
  const handleSave = () => {
    ...
    let existingObjectIndex = ConferenceData.findIndex(
      (object) => object.id === id
    );
    if (existingObjectIndex !== -1) {
      let updatedConferenceData = [...ConferenceData];
      updatedConferenceData[existingObjectIndex].title = title;
  ...
  return (
    <div
      className={`flex flex-col border-skill ${id}conf`}
      id={id}
    ...
    >
      <div className="ml-9 pb-3 educationAllFields" id="educationOldField">
        <div className="first">
          <p
            ...
            type="text"
            data-placeholder="Conference/Course Name"
            ref={InstRef}
        ...
        <div className="flex flex-col space-y-2" id="educationDetail">
          ...
          >
            ...
            />
            <h1
              className={`y text-[#A8B0B9] text-black ${
                FontSize == "md"
                  ? "text-[16px]"
                  : FontSize == "sm"
                  ? "text-[14px]"
          ...
          <input
            type="text"
            placeholder="Conference/Issuer of certificate"
            className={`text-black y ${
              FontSize == "md"
                ? "text-[16px]"
                : FontSize == "sm"
          ...
          />
          <div
            className={`grid grid-cols-${CoursesPerRowConf}`}
            id={`${id}ConfcourseId`}
          >
            <div
      ...
      <div className="flex items-center gap-2 hidden ml-3" ref={addConf}>
        <span
          onClick={handleAdd}
...
export default function Conference() {
  ...
  return (
    <div className="pb-3 pr-5 " key="conference">
      <div className="  ml-6 pt-5 px-5 ">
        <div className="flex space-x-6 relative right-5  items-center w-[450px] conference">
          {Icon && (
