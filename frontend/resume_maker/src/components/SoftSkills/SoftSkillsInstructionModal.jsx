...
export default function SoftSkillsInstructionModal(props) {
    const { Instruction, setInstruction } = useContext(ComponentListContext)
    const swiper = useSwiper();
    const SoftSkillsInstructionModalRef = useRef()
    const [slide, setslide] = useState(1)

    useEffect(() => {
        let handler = (e) => {
            if (SoftSkillsInstructionModalRef.current && !SoftSkillsInstructionModalRef.current.contains(e.target)) {
                setInstruction(false)
            }

        }
        document.getElementById('skillsInsModal').addEventListener('mousedown', handler)

    })

    return (

        <div className='ml-20 my-10' ref={SoftSkillsInstructionModalRef}>
