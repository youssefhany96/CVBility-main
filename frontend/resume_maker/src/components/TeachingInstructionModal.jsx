...
export default function TeachingInstructionModal(props) {
    const { Instruction, setInstruction } = useContext(ComponentListContext)
    const swiper = useSwiper();
    const TeachingInstructionModalRef = useRef()
    ...
    useEffect(() => {
        let handler = (e) => {
            // e.stopPropagation()
            // e.preventDefault()
            if (TeachingInstructionModalRef.current !== null && !TeachingInstructionModalRef.current.contains(e.target)) {
                console.log(TeachingInstructionModalRef.current.contains(e.target))
    ...
    return (
        ...
        <div className='ml-20 my-10' ref={TeachingInstructionModalRef}>
