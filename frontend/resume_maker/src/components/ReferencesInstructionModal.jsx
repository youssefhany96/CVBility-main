...
export default function ReferencesInstructionModal(props) {
  const { Instruction, setInstruction } = useContext(ComponentListContext);
  const swiper = useSwiper();
  const ReferencesInstructionModalRef = useRef();
  ...
  useEffect(() => {
    let handler = (e) => {
      ...
      // e.preventDefault()
      if (
        ReferencesInstructionModalRef.current !== null &&
        !ReferencesInstructionModalRef.current.contains(e.target)
      ) {
  ...
  return (
    <div className="ml-20 my-10" ref={ReferencesInstructionModalRef}>
