...
export default function AchievementInstructionModal(props) {
    const { Instruction, setInstruction } = useContext(ComponentListContext)
    const swiper = useSwiper();
    const AchievementInstructionModalRef = useRef()
    const [slide, setslide] = useState(1)

    useEffect(() => {
        let handler = (e) => {
            if (AchievementInstructionModalRef.current && !AchievementInstructionModalRef.current.contains(e.target)) {
    ...
    return (
        ...
        <div className='ml-20 my-10' ref={AchievementInstructionModalRef}>
