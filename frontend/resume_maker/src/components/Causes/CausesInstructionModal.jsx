...
export default function CausesInstructionModal(props) {
    const { Instruction, setInstruction } = useContext(ComponentListContext)
    const swiper = useSwiper();
    const CausesInstructionModalRef = useRef()
    const [slide, setslide] = useState(1)

    useEffect(() => {
        let handler = (e) => {
            if (CausesInstructionModalRef.current && !CausesInstructionModalRef.current.contains(e.target)) {
    ...
    return (
        ...
        <div className='ml-20 my-10' ref={CausesInstructionModalRef}>
