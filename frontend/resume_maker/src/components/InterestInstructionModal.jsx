...
export default function InterestInstructionModal(props) {
    const { Instruction, setInstruction } = useContext(ComponentListContext)
    const swiper = useSwiper();
    const InterestInstructionModalRef = useRef()
    const [slide, setslide] = useState(1)

    useEffect(() => {
        let handler = (e) => {
            if (InterestInstructionModalRef.current && !InterestInstructionModalRef.current.contains(e.target)) {
                setInstruction(false)
            }

        }
        document.getElementById('interestsInsModal').addEventListener('mousedown', handler)

    })

    return (

        <div className='ml-20 my-10' ref={InterestInstructionModalRef}>
