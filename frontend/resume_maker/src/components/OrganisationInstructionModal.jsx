...
export default function OrganisationInstructionModal(props) {
  const { Instruction, setInstruction } = useContext(ComponentListContext);
  const swiper = useSwiper();
  const OrganisationInstructionModalRef = useRef();
  ...
  useEffect(() => {
    let handler = (e) => {
      ...
      // e.preventDefault()
      if (
        OrganisationInstructionModalRef.current !== null &&
        !OrganisationInstructionModalRef.current.contains(e.target)
      ) {
        setInstruction(false);
      }
    };
    document
      .getElementById("educationInsModal")
      .addEventListener("mousedown", handler);
  });

  return (
    <div className="ml-20 my-10" ref={OrganisationInstructionModalRef}>
      <div className={`flex  space-x-10`}>
        <h1 className="text-[#070F18] font-[500]">Examples :</h1>
        <div className="bg-[#FBFCFF] rounded-md w-20 flex space-x-5">
          <button
            className={`bg-primary py-2 px-2 rounded-md hover:text-primary2`}
          >
            Junior
          </button>
          <button
            className={` py-2 px-2 text-[#A8B0B9] rounded-md hover:text-primary2`}
          >
            Intermediate
          </button>
          <button
      ...
      <div
        className={`border border-gray-600 w-[400px] h-[300px] mt-5 rounded-md`}
      >
        <div className="flex flex-col m-4 space-y-1">
          ...
          <li className="text-black text-sm">
            Entrepreurship and Global Starategy
          </li>
          <li className="text-black text-sm">
            International Economics Priciples{" "}
          </li>
              ...
              return (
                <div className="">
                  <h1 className="text-sm text-black">
                    Here you should include your school(s) and the courses that
                    you took which are relevant to the position you are applying
                    for.
                  </h1>
                </div>
              );
