...
export default function CertificatesInstructionModal(props) {
    const { Instruction, setInstruction } = useContext(ComponentListContext)
    const swiper = useSwiper();
    const CertificatesInstructionModalRef = useRef()
    const [slide, setslide] = useState(1)

    useEffect(() => {
        let handler = (e) => {
            if (CertificatesInstructionModalRef.current && !CertificatesInstructionModalRef.current.contains(e.target)) {
                setInstruction(false)
            }

        }
        document.getElementById('achievementsInsModal').addEventListener('mousedown', handler)

    })

    return (

        <div className='ml-20 my-10' ref={CertificatesInstructionModalRef}>
