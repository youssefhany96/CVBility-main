...
export default function EducationInstructionModal(props) {
    const { Instruction, setInstruction } = useContext(ComponentListContext)
    const swiper = useSwiper();
    const EducationInstructionModalRef = useRef()
    ...
    useEffect(() => {
        let handler = (e) => {
            // e.stopPropagation()
            // e.preventDefault()
            if (EducationInstructionModalRef.current !== null && !EducationInstructionModalRef.current.contains(e.target)) {
                console.log(EducationInstructionModalRef.current.contains(e.target))

                setInstruction(false)
            }

        }
        document.getElementById('educationInsModal').addEventListener('mousedown', handler)

    })

    return (

        <div className='ml-20 my-10' ref={EducationInstructionModalRef}>
