...
export default function SkillsInstructionModal(props) {
    const { Instruction, setInstruction } = useContext(ComponentListContext)
    const swiper = useSwiper();
    const SkillsInstructionModalRef = useRef()
    const [slide, setslide] = useState(1)

    useEffect(() => {
        let handler = (e) => {
            if (SkillsInstructionModalRef.current && !SkillsInstructionModalRef.current.contains(e.target)) {
    ...
    return (
        ...
        <div className='ml-20 my-10' ref={SkillsInstructionModalRef}>
