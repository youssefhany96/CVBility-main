...
export default function ConferenceInstructionModal(props) {
  const { Instruction, setInstruction } = useContext(ComponentListContext);
  const swiper = useSwiper();
  const ConferenceInstructionModalRef = useRef();
  ...
  useEffect(() => {
    let handler = (e) => {
      ...
      // e.preventDefault()
      if (
        ConferenceInstructionModalRef.current !== null &&
        !ConferenceInstructionModalRef.current.contains(e.target)
      ) {
  ...
  return (
    <div className="ml-20 my-10" ref={ConferenceInstructionModalRef}>
