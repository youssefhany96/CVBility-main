...
export default function LanguageInstructionModal(props) {
    const { Instruction, setInstruction } = useContext(ComponentListContext)
    const swiper = useSwiper();
    const LanguageInstructionModalRef = useRef()
    const [slide, setslide] = useState(1)

    useEffect(() => {
        let handler = (e) => {
            if (LanguageInstructionModalRef.current && !LanguageInstructionModalRef.current.contains(e.target)) {
                setInstruction(false)
            }

        }
        document.getElementById('languageInsModal').addEventListener('mousedown', handler)

    })

    return (

        <div className='ml-20 my-10' ref={LanguageInstructionModalRef}>
