...
export default function AwardsInstructionModal(props) {
  const { Instruction, setInstruction } = useContext(ComponentListContext);
  const swiper = useSwiper();
  const AwardsInstructionModalRef = useRef();
  ...
  useEffect(() => {
    let handler = (e) => {
      ...
      // e.preventDefault()
      if (
        AwardsInstructionModalRef.current !== null &&
        !AwardsInstructionModalRef.current.contains(e.target)
      ) {
  ...
  return (
    <div className="ml-20 my-10" ref={AwardsInstructionModalRef}>
