...
export default function VolunteerInstructionModal(props) {
  const { Instruction, setInstruction } = useContext(ComponentListContext);
  const swiper = useSwiper();
  const VolunteerInstructionModalRef = useRef();
  ...
  useEffect(() => {
    let handler = (e) => {
      ...
      // e.preventDefault()
      if (
        VolunteerInstructionModalRef.current !== null &&
        !VolunteerInstructionModalRef.current.contains(e.target)
      ) {
  ...
  return (
    <div className="ml-20 my-10" ref={VolunteerInstructionModalRef}>
