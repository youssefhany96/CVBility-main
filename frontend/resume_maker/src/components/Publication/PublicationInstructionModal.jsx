...
export default function PublicationInstructionModal(props) {
  const { Instruction, setInstruction } = useContext(ComponentListContext);
  const swiper = useSwiper();
  const PublicationInstructionModalRef = useRef();
  ...
  useEffect(() => {
    let handler = (e) => {
      ...
      // e.preventDefault()
      if (
        PublicationInstructionModalRef.current !== null &&
        !PublicationInstructionModalRef.current.contains(e.target)
      ) {
  ...
  return (
    <div className="ml-20 my-10" ref={PublicationInstructionModalRef}>
