...
export default function TechnicalInstructionModal(props) {
    const { Instruction, setInstruction } = useContext(ComponentListContext)
    const swiper = useSwiper();
    const TechnicalInstructionModalRef = useRef()
    const [slide, setslide] = useState(1)

    useEffect(() => {
        let handler = (e) => {
            if (TechnicalInstructionModalRef.current && !TechnicalInstructionModalRef.current.contains(e.target)) {
    ...
    return (
        ...
        <div className='ml-20 my-10' ref={TechnicalInstructionModalRef}>
